...
def inv_var_weight(mus, sigmas):
    ...
    inv_var_var = 1.0 / jnp.sum(1.0 / (sigmas**2), axis=0)
    # Calculate inverse variance weighted average
    inv_var_avg = jnp.sum(mus / (sigmas**2), axis=0) * inv_var_var
    # Convert weigthed variance to sigma
    inv_var_sigma = jnp.sqrt(inv_var_var)
    # And return results
    return inv_var_avg, inv_var_sigma

def softmax(x):
    """
    Applies softmax to tensors of inputs, using jax softmax funcion
...
def normalise(x):
    """
    Normalises vector of input to unit norm
    Assumes x is a 1D vector, or batches of row vectors with the batches along dim 0    
    """      
...
def relu(x):
    """
    Applies rectified linear activation unit to tensors of inputs, using jax relu funcion
    """     
    # Return jax relu    
...
def leaky_relu(x):
    """
    Applies leaky (meaning small negative slope instead of zeros) rectified linear activation unit to tensors of inputs, using jnp leaky relu funcion
    """         
    # Return jax leaky relu 
    return jax.nn.leaky_relu(x)

def squared_error(value, target):
    """
    Calculates mean squared error (L2 norm) between (list of) tensors value and target by using jnp MSE loss
    Get mean over last dimension to keep losses of different batches separate
    """        
    if isinstance(value, list):
        loss = [jnp.mean(value[i], target[i])**2 for i in range(len(value))]
    else:
        loss = jnp.mean((value - target)**2, dim=-1)
    return loss

def cross_entropy(value, target):
    """
    Calculates binary cross entropy between tensors value and target, assuming that value is an unnormalized logit 
    Get mean over last dimension to keep losses of different batches separate
    """                
    # TODO: implement with logsumexp trick
    if isinstance(value, list):
        loss = [-(targ * jnp.log(jax.nn.sigmoid(val)) + (1 - targ) * (1 - jnp.log(jax.nn.sigmoid(val)))) for val, targ in zip(value, target)]
    else:
        logit = jnp.log(jax.nn.sigmoid(value))
        loss = -(target * logit + (1 - target) * (1 - logit))
    return loss
...
def downsample(value, target_dim):
    ...
    downsample = np.zeros((value_dim,target_dim), dtype = float)
    # Fill downsampling matrix with chunks
    for curr_entry in range(target_dim):
        downsample[edges[curr_entry]:edges[curr_entry+1],curr_entry] = jnp.array(1.0/(edges[curr_entry+1]-edges[curr_entry]), dtype=float)
    # Do downsampling by matrix multiplication
    return jnp.matmul(value,downsample)
