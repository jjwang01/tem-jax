...
class World:
    def __init__(self, env, randomise_observations=False, randomise_policy=False, shiny=None):
        ...
        if isinstance(env, str) or isinstance(env, np.str_):
    ...
    def get_observation(self, new_location):
        ...
        # Create a new observation by converting the new observation to a jnp array 
        new_observation = jnp.array(new_observation, dtype=float).view((new_observation.shape[0]))
    ...
    def get_reward(self, reward_locations):
        ...
        reward_locations = [reward_locations] if not isinstance(reward_locations, list) else reward_locations
